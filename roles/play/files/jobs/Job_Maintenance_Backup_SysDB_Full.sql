...
IF EXISTS (SELECT job_id FROM msdb.dbo.sysjobs_view WHERE name = N'Maintenance - Backup - SysDB - Full')
EXEC msdb.dbo.sp_delete_job @job_name=N'Maintenance - Backup - SysDB - Full', @delete_unused_schedule=1
GO
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'Maintenance - Backup - SysDB - Full', 
		@enabled=1, 
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Maintenance - Backup - SysDB - Full', 
		@step_id=1, 
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'Maintenance - Daily at 4:00 AM', 
		@enabled=1, 
		@freq_type=4, 
		@freq_interval=1, 
		@freq_subday_type=1, 
		...
		@schedule_uid=N'9a4ed466-6593-47d8-9437-8ef188ea06ca'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
